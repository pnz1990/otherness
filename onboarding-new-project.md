...
```yaml
mode: "standalone"          # or "bounded-standalone" for concurrent scoped sessions
agents_path: "~/.otherness/agents"
max_parallel: 3             # max concurrent bounded sessions
```
...
The constitution governs agent behavior. Run `/speckit.constitution` to auto-generate, or copy from another otherness project. Keep Articles I–VI (core behavioral rules) and Article IX (docs-first principle). Add a project-specific architecture article for any constraint that must never be violated.
