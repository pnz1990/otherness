...
GitHub labels and the report issue were created automatically during onboarding. The \`_state\` branch is bootstrapped automatically on the first \`/otherness.run\` write if it doesn't exist. The agent will read these files and generate the first queue automatically."
```
