...
**The system derives its own validation criteria.** As a project accumulates design docs and batch history, otherness derives what "done" means from the current state of those docs — not from a static definition written at inception. The simulation predicts what healthy progress looks like. The PM phase measures reality against that prediction. The human does not need to define success; the system does. The human re-enters only when the system determines it cannot proceed without external judgment.
