...
| `meaningful_prs` | PRs merged this batch that reference a design doc (docs/design/, 🔲 →, or "design doc") — excludes chore/metrics/session-report PRs | ↑ (real work) |
| `session_items_completed` | Number of items (PRs) completed in this session before SM/PM gate fired; target ≥3 for healthy multi-item sessions | ↑ (≥3) |

---

## Batch Log

| Date | Batch | prs_merged | needs_human | ci_red_hours | skills_count | todo_shipped | time_to_merge_avg_min | vision_prs | session_outcome | arch_convergence | sim_floor_delta | queue_guard_fires | meaningful_prs | session_items_completed | Notes |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
