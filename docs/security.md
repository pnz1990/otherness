...
### Token scope: GitHub App per-repo token (M3) ✅

The agent uses a per-repository installation token generated by a GitHub App at the
start of every session. This token:
...
This closes attack vectors 3E (PAT cross-repo blast radius) and 4C (cross-repo
contamination). A compromised session on one project cannot affect your other projects.
...
### PAT fallback cross-repo blast radius ⚠️ (only if GitHub App is not configured)
