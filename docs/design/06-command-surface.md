...
- ✅ Automated `docs/aide/progress.md` update in SM §4f: SM now updates `progress.md` every batch with health signal, last-shipped PR, queue depth, and batch counter. Committed directly to main via pull-rebase-retry pattern. (issue-643, 2026-04-20)

## Future (🔲)

- 🔲 `/otherness.arch-audit` must run on a scheduled cadence and feed findings into the work queue: today, `arch-audit` is a human-triggered INTERNAL command — it runs when a human remembers to run it. Between runs, architectural drift accumulates undetected. SM §4e must schedule an arch-audit run every `N` SM cycles (default: 20, configurable via `otherness-config.yaml` as `audit.cadence_cycles`). After the run completes, SM §4e must parse the audit output for actionable findings (lines matching `[FINDING]`, `[GAP]`, or `[CLAIM-MISMATCH]`) and create GitHub issues for each finding not already represented by an open issue (deduplicated by first 50 chars of finding text). The issues must be labeled `kind/bug area/agent-loop priority/medium` and enter the normal COORD claim queue. Currently, arch-audit findings are human-read and human-acted-on or silently discarded — the self-improvement loop has a dead end where the most deep diagnostic tool produces output that never reaches the queue. ⚠️ Inferred from self-improvement lens: `/otherness.arch-audit` is an internal command that runs ad-hoc and produces findings; there is no mechanism converting those findings into queue items or scheduling the audit to run on its own cadence.

- 🔲 Arch-audit findings must be tracked through to closure: when SM §4e creates a GitHub issue from an arch-audit finding, the issue number must be written to `_state:arch-audit-findings.json` as `{finding_hash: N, issue_number: M, created_at: date, closed_at: null}`. Each subsequent arch-audit run must re-check: for each finding still open in the JSON, is the GitHub issue now closed with a merged PR? If yes: write `closed_at` timestamp and `closed_by_pr: N`. If a finding has been open >30 days with no PR merged: SM §4e must escalate to `priority/high`. Without this tracking, arch-audit creates issues but never knows if the underlying architectural problem was actually fixed — the audit result is filed and forgotten. A finding that is never confirmed closed may still be present in the codebase even after a PR is merged (the PR may have partially addressed it). ⚠️ Inferred from self-improvement lens: arch-audit findings are not verified as acted on; the feedback loop from audit finding to architectural improvement has no closure signal.

- ✅ README health badge and "last shipped" line: a human visiting the repo on GitHub.com has no at-a-glance signal of system health before they open any file or run any command. The README must include: (1) a GitHub Actions workflow badge pointing to the `otherness-scheduled` workflow (green = running, red = failing), (2) a single line updated by SM §4f every batch: `**Last shipped:** <PR title> (<date>)` — same data as `docs/aide/progress.md` Current State but visible without navigating into docs. Implementation: SM §4f uses `sed` to replace the badge line and last-shipped line in `README.md`. The badge is already supported by GitHub natively (`![CI](https://github.com/owner/repo/actions/workflows/otherness-scheduled.yml/badge.svg)`). This gives the human a 2-second health check from the repo homepage without opening a terminal. ⚠️ Inferred from visibility lens: a human looking at GitHub right now cannot quickly tell if the system is healthy or what it shipped today — no at-a-glance signal on the repo homepage. (PR #TBD, 2026-04-21)
