...
- 🔲 PM competitive comparison must produce a verifiable rubric artifact, not just `⚠️ Inferred` stubs: PM §5c writes competitive gap findings as design doc stubs. But no artifact exists that defines WHAT the comparison is measuring — so a human cannot tell if otherness is catching up to or falling behind its comparators. AGENTS.md specifies the PM must compare otherness against "Hermes, Multica, Archon" every batch. The comparison currently produces ad-hoc `⚠️ Inferred` items with no consistent structure. PM §5c must produce a `docs/aide/competitive-standing.md` file (written and maintained like `metrics.md`) with a fixed rubric: four dimensions evaluated per comparison run — (1) core-loop reliability (does the comparator ship PRs every run?), (2) self-improvement (does the comparator's agent get smarter over time?), (3) onboarding friction (can a new project start in <30 minutes?), (4) visibility (can a human read system health in 30 seconds?). For each dimension: `otherness_score (0-3)` and `comparator_score (0-3)`. PM §5c must append one dated row per 10-batch comparison cycle. SM §4b must read the file and include a one-line competitive delta in the health comment: "vs. spec-kitty: +1 reliability, -1 visibility (last checked: N batches ago)". Without this rubric, competitive observations are unmeasurable assertions — the human cannot tell whether otherness is gaining or losing ground against the systems it was designed to outperform. ⚠️ Inferred from self-improvement and honesty lenses: the agents compare against competitors but produce no artifact that makes the comparison verifiable or trackable over time; a flat narrative "spec-kitty is simpler" adds no persistent signal to the system; only a structured scored rubric produces a measurable trend.

- 🔲 Vision evolution cadence must have a defined "pressure addressed" criterion that makes the lens bullet for onboarding and visibility SCAN-5-scorable: SCAN 5's bullet-scoring (doc 37, §37.2) checks whether each pressure lens bullet is "addressed" by counting Present items that mention its key nouns. The onboarding lens ("Is the onboarding good enough?") scores 0 matches because no key noun from "onboarding" appears in any Present item header — the Present items in doc 32 use the word "onboarding" but SCAN 5's noun extraction returns empty. Similarly the visibility lens ("Is the visibility good enough?") scores 0 because "visib" is not a common Present item term. This means both lenses will remain permanently unaddressed by SCAN 5 scoring regardless of how many onboarding and visibility items ship. The fix has two parts: (1) `vibe-vision-auto.md` SCAN 5 must expand its noun extraction to include: `{'onboard', 'setup', 'dashboard', 'health', 'visib', 'progress', 'status', 'report', 'schema', 'first-run', 'setup-guide'}` — concrete terms from the onboarding and visibility domains; (2) COORD must prioritize shipping items from doc 32 and doc 39 until at least 2 Present items exist in each doc using those terms, so that future SCAN 5 runs can correctly score those lenses as addressed. The current state — both lenses permanently at 0% — means SCAN 5 will never recommend a pressure rewrite even when substantial work ships, and the pressure context remains frozen for these two areas indefinitely. ⚠️ Inferred from visibility lens: the SCAN 5 scoring mechanism is broken for abstract lens bullets whose key nouns are not present in the noun-extraction dictionary; the gap is not in the work being done but in the scoring's ability to recognise it.
