...
- 🔲 45.3 — Board status and milestone: fix the gap (27 open issues with no milestone today). The COORD issue-creation block must include `gh issue edit --milestone` and `gh project item-add` as non-optional, always-running commands.
...
- 🔲 45.7 — Step A timeout must not consume Step B's runner budget — a vision scan that runs over 30 minutes silently kills the build session: the `otherness-scheduled.yml` workflow allocates a single GitHub Actions job with a default `timeout-minutes: 360` for the full run. Step A (vibe-vision-auto) has no per-step timeout. If Step A consumes 45+ minutes (slow LLM, large design-doc corpus, iterative scan loops), Step B (the actual implementation run) receives only the remaining job budget — potentially 10–15 minutes — which is insufficient to claim an item, implement it, open a PR, and run CI. Step B then exits mid-session with no PR shipped. The scheduled run shows green (no failure) while delivering nothing. `otherness-scheduled.yml` must: (1) add `timeout-minutes: 25` to the Step A workflow step — a vision scan that cannot complete in 25 minutes is too expensive and must be cut short; (2) Step A must handle the timeout gracefully: if SCAN 3 or SCAN 5 is interrupted mid-execution, SCAN 1 and SCAN 2 results (already committed) are preserved; (3) Step B must receive a minimum of 60 minutes — enforce this by adding a pre-Step-B check: `elapsed=$(( $(date +%s) - START_TIME )); remaining=$(( JOB_TIMEOUT - elapsed )); if [ $remaining -lt 3600 ]; then echo "::warning::Step B has <60 minutes remaining — skipping to preserve budget"; exit 0; fi`. Without this guard, every vision scan that runs long silently eliminates the implementation run from the same scheduled trigger. A session that looks green in the Actions tab but shipped 0 PRs is the "silent failure" the reliability lens names. ⚠️ Inferred from reliability lens: sessions fail silently; one specific cause is the dual-step workflow's lack of per-step time budgets; Step A can exhaust the run budget without any error signal.

- 🔲 45.8 — Phase file line count must be tracked and capped — reliability degrades as instruction length grows: `45-distil-and-simplify.md` identifies sm.md at 5,015 lines as the primary complexity debt. But no ongoing mechanism prevents it from growing back after a reduction. SM §4a must, every 10 batches, check the line count of each phase file (`agents/phases/coord.md`, `agents/phases/eng.md`, `agents/phases/qa.md`, `agents/phases/sm.md`) and write the counts to `state.json` as `phase_line_counts`. SM §4a must flag when any phase file exceeds its cap: sm.md > 2,500 lines = AMBER; coord.md > 1,500 lines = AMBER; eng.md > 2,000 lines = AMBER; qa.md > 1,000 lines = AMBER. When any cap is exceeded: open a `kind/chore priority/high` issue: "Phase file `[file]` has grown to [N] lines — exceeds the [cap] line cap. Simplification cycle must run before new instructions are added." The caps are the output of the 45.1 reduction — they encode the target state as a guardrail. Without a cap, a simplified 2,000-line sm.md will be at 3,500 lines within 20 batches as new instructions are added without any corresponding removal. The line count cap is the enforcement mechanism that makes simplification permanent rather than a one-time cleanup. ⚠️ Inferred from reliability lens: the system adds instructions faster than it removes them; without a hard cap enforced per-batch, the post-simplification state cannot be maintained; each [AI-STEP] added without a cap grows the next simplification debt silently.
