...
- ✅ Simulation prediction DIFF — SM §4e now: (1) saves existing `sim-prediction.json` as `sim-prediction-prev.json` before overwriting; (2) computes `prediction_diff` block (`floor_delta`, `ceiling_delta`, `arch_convergence_delta`, `recovery_action_changed`, `prev_calibrated_at`) and includes it in `sim-prediction.json`; (3) SM §4f health table includes "Sim delta: floor ±N | ceiling ±M | conv ±K | action →curr" row. (PR #833, 2026-04-22) ⚠️ Stale — referenced file not found
...
- ✅ Simulation predictions visibly change agent behavior: SM §4e divergence detection computes `recovery_action` field (`trigger_learn` | `escalate_oldest_needs_human` | `prioritize_ci_fix` | `trigger_vision_synthesis` | `none`) based on queue depth, needs-human count, CI failure count, and consecutive below-floor count; writes it to `_state:sim-prediction.json`; `coord.md §1b-sim` reads it at session start, logs the action, escalates oldest needs-human issue when indicated, and adjusts claim sort key to prefer `kind/bug` items when `recovery_action=prioritize_ci_fix`. (PR #680, 2026-04-21)
- ✅ `SM §4e-iii`: simulation-behavior coupling gate — after each calibration, verifies that arch_convergence AMBER (≥0.7) signals triggered a learn session (checks PROVENANCE.md, learn branches, learn issues); verifies that a learn session actually decreased arch_convergence in the subsequent calibration; posts `[⚠️ Simulation loop unclosed]` on REPORT_ISSUE if the chain never closed; runs at calibration cadence (every N SM cycles); informational only, does not block work (PR #668, 2026-04-20)
- ✅ SM health signal distinguishes real GREEN from stall-GREEN: `SM §4f` PROGRESS_CLASS now uses ADVANCING/STALLING/STALLED based on consecutive `meaningful_prs == 0` batches in metrics.md. STALLING means 1-2 consecutive batches with 0 meaningful PRs; STALLED means 3+. When STALLING or STALLED and HEALTH=GREEN, health is upgraded to AMBER. GREEN now only fires when meaningful work shipped this session. (PR #698, 2026-04-21)
- ✅ Simulation calibration staleness visible in health signal: `sim-prediction.json` stores a `calibrated_at` timestamp but this is never surfaced to the human. SM §4f health comment must include "Sim calibrated: N days ago" and downgrade to AMBER if calibration is >14 days old. A prediction based on 14-day-old data may be systematically wrong — the human should know when the anchor is stale. Implementation: SM §4f reads `calibrated_at` from `_state:sim-prediction.json`, computes age, appends to health comment, and sets `progress_signal: AMBER` if age > 14 days. ⚠️ Inferred from honesty lens: simulation exists but its predictive authority is invisible; a stale calibration silently produces misleading divergence signals. (PR #706, 2026-04-21)
- ✅ `sim_floor_delta` feeds into `recovery_action` computation: SM §4e-ii reads the last 5 `sim_floor_delta` values from `metrics.md`, computes a rolling average; if avg ≤ −1.5 for 3 consecutive batches (`sim_floor_delta_negative_streak` in `sim-prediction.json`), `recovery_action` is set to `trigger_vision_synthesis`. Fail-open. (PR #737, 2026-04-21)
- ✅ Simulation prediction DIFF already shipped — see Present §above (PR #833, 2026-04-22)
- 🔲 `prediction_diff` must drive behavior change in COORD — not just be logged in the health table: `sim-prediction.json` now includes `prediction_diff.arch_convergence_delta` and `prediction_diff.recovery_action_changed` (PR #833). The health table shows "Sim delta: floor ±N | ceiling ±M | conv ±K | action →curr". But the `prediction_diff` is currently display-only — COORD §1b-sim reads `recovery_action` but does NOT read `prediction_diff`. A session where `arch_convergence_delta > +0.1` (convergence worsening fast) should elicit a different COORD response than a session where the delta is near zero. COORD §1b-sim must: (1) read `arch_convergence_delta` from `sim-prediction.json`; (2) if `arch_convergence_delta > +0.1` AND `frame_lock_detected=false`: set `frame_lock_detected=true` preemptively — convergence is worsening fast enough to treat as early frame-lock; (3) if `recovery_action_changed=true` (the recommended action shifted this calibration): log at session start "Sim recovery_action changed: <prev> → <curr> — adjusting session strategy" on the report issue. Without these two steps, the `prediction_diff` is computed and displayed but never causes any behavioral difference in the session that reads it. A diff that influences nothing is a dashboard element, not an instrument. ⚠️ Inferred from honesty lens: the simulation exists but its predictions are not visibly changing agent behavior — PR #833 added the diff to the health table display, but COORD still only reads `recovery_action`, not the rate-of-change signals that would make it respond earlier to worsening trends.
...
- ✅ Simulation prediction accuracy score must be tracked — the simulation is not useful if its floor predictions are systematically wrong: `sim-prediction.json` writes a `prs_next_batch_floor` every calibration cycle. After each batch, SM §4e compares `actual_shipped` to `prs_next_batch_floor`. But there is no running accuracy score. A simulation that predicts floor=5 while actual is consistently 1 is not calibrated — it is blindly optimistic. Its divergence signals are noise. SM §4e must maintain a `prediction_accuracy_rolling_10` field in `sim-prediction.json`: count of the last 10 batches where `actual_shipped >= prs_next_batch_floor` divided by 10. If accuracy drops below 60% (less than 6 of 10 predictions correct): SM must include in the health comment: "⚠️ Sim accuracy: N/10 — predictions unreliable. Recalibration needed." and set `recovery_action=recalibrate` in the next cycle. A simulation that is wrong 5 times out of 10 has no predictive authority. Without this score, the system cannot tell whether its self-model is calibrated or systematically biased — and a biased model produces false confidence in the divergence signal. ⚠️ Inferred from honesty lens: the simulation exists but its predictions are not visibly changing agent behavior — partly because there is no mechanism confirming the predictions are accurate; a simulation with 40% accuracy is worse than no simulation because it produces misleading recovery actions. (PR #833)

- ✅ SM GREEN health signal must be directly gated on simulation floor — actual < floor must produce AMBER immediately, not after 3 consecutive divergences: the current divergence detection (SM §4e) tracks consecutive below-floor batches and posts a `[⚠️ Simulation divergence]` comment after 3 in a row — but SM §4f still writes `health: GREEN` for the first two. This means the health signal says GREEN while the simulation is actively predicting that the session underperformed. The simulation is supposed to be the anchor; a GREEN signal that ignores a below-floor result is not anchored. SM §4f must: (1) read `prs_next_batch_floor` from `_state:sim-prediction.json`; (2) read `meaningful_prs` from the current batch; (3) if `meaningful_prs < prs_next_batch_floor` AND floor > 0: set `health: AMBER` immediately, with note "⚠️ AMBER — actual=M meaningful PRs, floor prediction was N"; (4) retain the existing 3-consecutive-divergence escalation path for [NEEDS HUMAN] escalation, but the per-batch AMBER must fire on the first below-floor result. The current design makes the simulation predictive in theory but non-binding in the health signal — 2 consecutive below-floor sessions both report GREEN. This is the gap between "the simulation produces a number" and "the simulation constrains the health verdict." ⚠️ Inferred from honesty lens: the loop is not honest enough — the SM health signal says GREEN but the products it manages are not advancing fast enough; the simulation predicts the floor but the health signal is not gated on that prediction, making the simulation decorative rather than authoritative. (PR #701)
