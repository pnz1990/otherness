...
- 🔲 Simulation prediction DIFF — show what changed between this calibration and the last: `sim-prediction.json` is overwritten every calibration cycle. There is no record of what the previous prediction said. A human reading the current prediction cannot tell whether the system's self-model has drifted, improved, or oscillated. SM §4e must maintain a `sim-prediction-prev.json` (copy of `sim-prediction.json` before each overwrite). After calibration, SM §4e must compute and include a `prediction_diff` block: changes in `floor`, `ceiling`, `recovery_action`, and `arch_convergence` from previous to current. SM §4f health comment must include: "Sim delta: floor ±N | ceiling ±M | convergence ±K | action: prev→curr". This converts the simulation from a snapshot instrument into a trajectory instrument — the human sees not just where the system thinks it is, but whether the self-model is stable or shifting. A simulation that produces identical predictions for 20 batches is calibrated against a stable system. One that oscillates between floor values signals that the real system is not matching the model. Without the diff, both look the same. ⚠️ Inferred from honesty lens: the simulation exists but its predictions are not visibly changing agent behavior — because the human cannot see whether the prediction itself is changing or stable; a static self-model is evidence the system has stopped learning, not evidence the system is healthy.
