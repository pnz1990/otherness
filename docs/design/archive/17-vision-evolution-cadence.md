...
- ✅ SCAN 5 domain-noun expansion: `_extract_bullets` now extracts concrete domain nouns (`onboard`, `visib`, `health`, `dashboard`, `metrics`, etc.) from pressure bullet text instead of using the first-30-char prefix; per-bullet scoring matches any domain noun against evidence items, making abstract lens bullets (onboarding, visibility) correctly scorable. Fallback to first-30-char prefix preserved for bullets with no domain noun. (PR #928, 2026-04-22)
