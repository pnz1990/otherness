...
## Future (🔲)
- ✅ ⚠️ Inferred: spatial collision detection — coord.md §1e [AI-STEP] converted to executable Python: AREA_TO_SPACES map + overlap check in candidate selection, file_spaces computation in claim block (PR #298, 2026-04-20)
- ✅ Concurrent session detection and deduplication guard: the heartbeat and file-space declaration model assumes sessions are well-behaved. But if two sessions start within the same 60-second cron window (GitHub Actions scheduling jitter, or a manual trigger concurrent with scheduled run), both sessions read an identical `state.json` before either writes their heartbeat — so both see an empty claim space and both claim the same item. The branch-lock (`git push origin feat/<item>`) catches this at the final step, but only after both sessions have already spent time analyzing the item. `coord.md §1b` must add a concurrent-session guard: on startup, read the `active_sessions` list from `state.json` (heartbeats updated in last 5 minutes). If ≥1 other session is active AND its heartbeat is <2 minutes old: wait 90 seconds and re-read before proceeding. If still concurrent after the wait: stagger by reading `session_id` (a UUID written to state.json at startup) and claiming only items whose `item_id mod 2 == session_id mod 2`. This prevents two sessions from claiming the same item even if they start simultaneously. ⚠️ Inferred from reliability lens: scheduling jitter can cause two sessions to start simultaneously; no guard prevents both from claiming identical items before either writes its heartbeat. (PR #731, 2026-04-21)
