...
- 🔲 41.3 — validate.sh check [8/8]: ✅ Present items referencing `state.json` fields must be verifiable: add a new check to `scripts/validate.sh` that parses `docs/design/*.md` for ✅ Present items containing `` `state.json` `` references and verifies the `_state` branch `state.json` contains those key names. The check uses the repo's `_state` branch via `gh api`. If the field is absent: validate.sh exits with a clear message: "Design doc 35: ✅ Present claims state.json.session_outcome exists — not found in _state branch." This makes the gap a build-time failure, not a silent runtime gap. ⚠️ Inferred from honesty lens: design docs can claim ✅ Present without the implementation ever running; the build must catch this.
