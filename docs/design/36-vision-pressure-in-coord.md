...
- ✅ 36.3 — COORD §1b: log vision-pressure claim decisions — after each claim, posts to REPORT_ISSUE: "Claimed #N [vision-backed: yes/no] — matched VPS key '<key>' OR no VPS key match". Non-blocking (|| true). VPS_MATCH_EOF updated to output matched key via pipe separator. (PR #issue-857, 2026-04-23)
...
- ✅ 36.5 — SM §4f: report vision pressure utilisation — in the health comment, include: "Vision-backed items claimed this session: N / M total claims." COORD §1e increments `VISION_BACKED_CLAIMS_THIS_SESSION` and `TOTAL_CLAIMS_THIS_SESSION` after each claim; SM §4f reads these env vars and adds a `| Vision pressure | N/M vision-backed claims | session |` row to the health comment table. Fail-open: if either var is unset, displays `?/? (untracked)`. (issue-944, 2026-04-23)
