...
- 🔲 28.1 — Scheduled workflow YAML syntax must be validated after every Step A commit: `vibe-vision-auto.md` SCAN 5 (doc 37) rewrites the "Context for this vision scan:" multi-line YAML string inside `.github/workflows/otherness-scheduled.yml`. Injecting multi-line strings into YAML is error-prone — incorrect indentation, an unescaped special character, or a misplaced pipe `|` breaks the YAML parse and silently disables the next run (GitHub Actions skips steps when workflow YAML is invalid). `scripts/validate.sh` currently checks that the scheduled workflow file exists and has ≥2 opencode steps — but it does NOT validate YAML syntax. Two changes required: (1) `scripts/validate.sh` must add a YAML syntax check for `.github/workflows/otherness-scheduled.yml` using Python's `yaml.safe_load` on the file content — any `yaml.YAMLError` must exit 1 with a clear message: "FAIL: otherness-scheduled.yml has invalid YAML syntax — likely caused by a broken SCAN 5 pressure rewrite. Restore the file or fix the indentation."; (2) `agents/vibe-vision-auto.md` SCAN 5 must, after writing the new pressure block, run `python3 -c "import yaml,sys; yaml.safe_load(open('.github/workflows/otherness-scheduled.yml'))" 2>&1` and abort the commit if the result is non-zero — reverting to the previous pressure block and logging the error to the report issue. Without these checks, a corrupt pressure rewrite by SCAN 5 silently breaks the scheduled workflow for the next run — the human sees no error (the commit passed CI), no batch runs, the `_state` branch goes stale, and the 72-hour liveness check fires a day later. The entire loop is broken by a YAML indentation error that could have been caught in 100ms. ⚠️ Inferred from reliability lens: sessions still fail silently; the SCAN 5 pressure rewrite touches YAML files but no validation step guards against syntax errors introduced by the rewrite; a broken scheduled workflow is a silent failure that takes 72+ hours to surface via the liveness check.
