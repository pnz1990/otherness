...
- ✅ Graceful partial handoff on GitHub Actions job timeout: `timeout-minutes: 330` set on the `otherness` job; cleanup step "Graceful timeout cleanup" added with `if: always()`, resets in-progress/assigned items to todo, posts warning comment on report issue (PR #633, 2026-04-21)

## Future (🔲)
- 🔲 Workflow-level silent failure detection: the agent relies on the scheduled GitHub Actions workflow running. But the workflow itself can fail silently before the agent starts — OIDC credentials expired, quota exhausted, runner unavailable, workflow manually disabled, or `anomalyco/opencode/github@latest` action not resolving. When this happens, no heartbeat is written, no issue is opened, and the system appears idle. SM §4a monitors `_state` for stale heartbeats within the agent loop — but has no mechanism to detect "the workflow never ran at all." The fix: the `otherness-scheduled.yml` must include a pre-flight step (runs before the agent step) that uses `GITHUB_TOKEN` (not `GH_TOKEN`) to post a one-line comment on the report issue: "🔄 Session started — YYYY-MM-DDTHH:MM:SSZ". This step runs unconditionally. If it succeeds but the agent step fails: a human sees "session started" with no corresponding "batch complete" comment and knows the runner reached the workflow but the agent crashed. If even this step is absent for >12 hours on an hourly-cron project: the workflow is disabled or failing before execution — PM §5 must detect this from `_state` inactivity and open a `[NEEDS HUMAN]` issue. ⚠️ Inferred from reliability lens: silent workflow failures are invisible to the agent loop; the system cannot distinguish "running but shipping nothing" from "not running at all."
- 🔲 Workflow disabled detection from within the loop: when a scheduled workflow is disabled via the GitHub UI, no `workflow_run` events fire and `_state` goes stale. PM §5j must detect `_state` last-updated age > 2× the configured cron interval; if so, attempt `gh workflow list --repo $REPO --json name,state` to check if the workflow is disabled. If disabled: open `[NEEDS HUMAN: workflow-disabled]` issue: "Scheduled workflow appears disabled. Re-enable at: <URL>". This is distinct from a transient runner failure — it requires human action to fix. Currently PM §5j only checks for stale `_state` and posts a generic stale warning; it does not distinguish the disabled-workflow root cause. ⚠️ Inferred from reliability lens: a system that relies on scheduled execution must be able to detect when scheduling itself has been interrupted.
