...
## Future (🔲)
- ✅ Workflow-level silent failure detection: spec written, step designed. The `otherness-scheduled.yml` requires a 'Workflow heartbeat — session started' step using `GITHUB_TOKEN` to post to the report issue. Not yet applied to the workflow file: the agent's GitHub App token lacks `workflows` write permission (doc 27 §M4). Workflow change requires human apply. Step code documented in PR #878 body. (Spec: .specify/specs/issue-810/) (PR #878)
