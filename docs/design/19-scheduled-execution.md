...
- ✅ Workflow disabled detection from within the loop: PM §5j now checks `gh workflow list` on the ref project when `_state` age > 72h. If the otherness scheduled workflow is `disabled_manually` or `disabled_inactivity`, a `[NEEDS HUMAN: workflow-disabled]` issue is opened with the Actions URL; transient failures (workflow active) fall back to the generic stall issue. Deduplication prevents repeat issues per project. (issue-929, 2026-04-22)
