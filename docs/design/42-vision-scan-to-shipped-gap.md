...
- ✅ 42.4 — `vibe-vision-auto.md` SCAN 5 time-based staleness trigger: SCAN 5 now fires a rewrite reminder after >30 days regardless of keyword match rate. Detects age via `<!-- pressure-rewritten: YYYY-MM-DD -->` comment in workflow; treats absence as stale. Only fires when keyword-match threshold hasn't already fired (to avoid duplicate items). Adds `🔲 Rewrite vision pressure context (time-based staleness)` Future item to the first matching design doc. (PR #987, 2026-04-23)

---

## Future (🔲)

- ✅ 42.2 — Vision scan gap stagnation ratio: each SCAN run computes `new_gaps`, `gaps_aged_30d`, `gaps_shipped`; ratio = aged/(new+shipped). Ratio >2.0 triggers `[⚠️ GAP STAGNATION: N old gaps]` in report comment. Metrics always reported for observability. (PR #865, 2026-04-23)
...
- ✅ 42.4 — shipped (see Present section above)
