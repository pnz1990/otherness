...
- 🔲 PROVENANCE.md cross-session topic balance analysis: PROVENANCE.md records every learn session's source and topic, but it is never read back to evaluate the balance of learning across paradigms. The result: SM §4c triggers learn sessions based on PROVENANCE.md date only (freshness check) but not on topic diversity. If the last 5 learn sessions all touched error handling, state management, or distributed systems, the agent's conceptual vocabulary in those areas grows while UI patterns, domain modelling, and test design atrophy. SM §4c must run a topic-balance analysis every 10 batches: parse PROVENANCE.md entries from the last 6 months, extract topic categories (via keyword clustering from entry titles), identify the most overrepresented category (highest frequency) and the most underrepresented (lowest frequency or absent). The NEXT learn session trigger must include a `topic_target: <underrepresented_category>` constraint, explicitly directing the learn agent toward the gap. A self-improvement loop that only learns more of what it already knows is not improving — it is narrowing. ⚠️ Inferred from self-improvement lens: learn sessions are triggered by staleness (date) not by topical imbalance; the agent's growing vocabulary is not directionally guided.
