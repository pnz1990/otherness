...
- 🔲 SM §4b must distinguish metrics-only PRs from real work PRs: the `prs_merged` column counts every merged PR, including the SM's own state commits and metrics-only PRs (e.g. a PR that only touches `docs/aide/metrics.md` or `docs/aide/progress.md`). A session that ships "3 PRs merged" where all 3 are SM housekeeping commits is reported as GREEN with `prs_merged: 3`. SM §4b must filter out PRs where the only changed files are under `docs/aide/` or `_state` — these are infrastructure PRs, not deliverables. The `meaningful_prs` column (spec'd in doc 21) must exclude them. The filter: a PR is "meaningful" if it changes ≥1 file outside `{docs/aide/, .otherness/state.json, _state branch files}`. A session where `meaningful_prs == 0` but `prs_merged > 0` (all housekeeping) must still trigger the AMBER health signal — shipping housekeeping is not shipping value. ⚠️ Inferred from reliability lens: the system produces housekeeping PRs with no real feature content and the health signal cannot distinguish them from feature PRs; a reliable system ships at least one meaningful PR every single run without exception.
...
- 🔲 In-session recovery level must be observable in the `[DEFECT]` comment and health signal: the session minimum meaningful-PR contract (above) specifies a 3-level recovery sequence. But it does not specify how the operator knows which level was reached before the exit. The `[DEFECT]` comment posted on exit must include the recovery trace: "Recovery level reached: [Level 1: second-chance item attempted | Level 2: vision synthesis attempted | Level 3: all paths exhausted]". SM §4b must write `recovery_level_reached: 0|1|2|3` to the metrics row for that batch. A `recovery_level_reached: 0` (no recovery attempted, just exited) is a bug in COORD; a `recovery_level_reached: 3` is the expected worst case. Without this trace, the operator has no way to distinguish "the system tried 3 recovery levels and still couldn't ship" (acceptable) from "the system gave up immediately" (a defect). The metric also enables detecting COORD bugs: if `recovery_level_reached` is always 0 across multiple sessions, COORD is not implementing the contract. ⚠️ Inferred from reliability lens: the in-session recovery contract is specified but its execution is invisible — a session that claims to have exhausted all three recovery paths has no observable proof of this, making the reliability guarantee unverifiable.
