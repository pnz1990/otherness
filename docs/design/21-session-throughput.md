...
- 🔲 COORD §1e must refuse to claim a `kind/chore` item when no `kind/enhancement` or `kind/bug` item has shipped this session — proactive queue enrichment as a reliability gate: the session minimum meaningful-PR contract (35.x) opens a `[DEFECT]` issue after the session ends with 0 meaningful PRs. But the defect detection is retrospective — it fires after the session fails. A more effective guarantee operates at claim time: COORD §1e must check, before claiming any item, whether `meaningful_prs_this_session == 0` AND the candidate item is `kind/chore`. If both are true: COORD must NOT claim the chore; instead, it must: (1) scan `docs/design/*.md` for an unissued `🔲 Future` item in a `## Future` section that has not yet been converted to a GitHub issue (no match in open issues for first 40 chars); (2) create a GitHub issue for that item; (3) claim the newly-created issue as the next work item. This converts the "guaranteed one meaningful PR per session" from a post-hoc detection into a proactive guarantee: the session cannot exit having shipped only chores unless there are literally no non-chore items available to generate. The fallback (if no unissued Future items exist): claim the chore normally, but write `meaningful_pr_guarantee_failed: true` to `state.json` so SM §4b can diagnose "queue is chore-saturated." Without this gate, the system can claim 5 chores in a row, each time passing the "is there a non-chore?" check that only looks at the existing queue — while 50 unissued design doc Future items sit unqueued. ⚠️ Inferred from reliability lens: sessions produce housekeeping PRs with no real feature content; the root cause is that the non-chore check only looks at already-queued items while design doc Future items can remain un-queued indefinitely; closing this gap at claim time rather than detection time guarantees the minimum.
...
- ✅ Session pre-flight gate (PREFLIGHT_CHECK): `coord.md §1b.5` — single atomic read of `housekeeping_streak`, `recovery_action`, `frame_lock_detected`, `silent_session_count`, `directive`; prints `Preflight: streak=N | recovery=<val> | frame_lock=<bool> | silent=N | directive=<val>`; produces `PREFLIGHT_PASS` or `PREFLIGHT_HOLD <reason>`; HOLD posts to REPORT_ISSUE and exits cleanly; fail-open on missing state. (PR #TBD, 2026-04-22)
...
- ✅ Health signal degrades to AMBER when 0 meaningful PRs shipped: SM §4f §0-meaningful-PRs-honesty-gate checks `MEANINGFUL_PRS` before finalising the health signal; if `MEANINGFUL_PRS == 0` AND `HEALTH == GREEN`: sets `HEALTH=AMBER` with reason "⚠️ AMBER — 0 meaningful PRs this session (chore-only or zero-ship)"; does not override RED; fail-open (unset MEANINGFUL_PRS treated as non-zero). (PR #870, 2026-04-22)


- 🔲 Silent session definition must be standardized and universally enforced: the term "silent session" is used in multiple places (SM §4b `silent_session_count`, COORD §1b preflight, `[DEFECT]` comment) but there is no single canonical definition of what constitutes a "silent session" — and the definitions used in different phase sections are inconsistent. Some sections define it as "0 PRs merged," others as "0 meaningful PRs," others as "COORD claimed but ENG never committed." An operator reading the health comment cannot tell which definition produced the metric they are seeing. SM §4b must establish a single canonical definition and enforce it everywhere: "A silent session is one where COORD ran, claimed at least 1 item (`items_claimed >= 1`), and the session exited with `meaningful_prs == 0`." Sessions where COORD ran but claimed 0 items (empty queue) are NOT silent sessions — they are empty-queue sessions (a different failure mode). Sessions where COORD did not run (full SM/PM session) are NOT silent sessions — they are maintenance sessions. The canonical definition must be written as a comment in `standalone.md §DEFINITIONS` and referenced by SM §4b, COORD §1b, and PM §5 by name. Without a canonical definition, "silent session count = 3" means three different things depending on which phase wrote it — making the metric unusable for root-cause analysis. ⚠️ Inferred from reliability lens: sessions fail silently; the current session failure detection is fragmented across three phase files with inconsistent definitions; a unified canonical definition is the prerequisite for all other silent-session detection mechanisms to agree on what they are measuring.
