...
- ✅ COORD §1e must refuse to claim a `kind/chore` item when no `kind/enhancement` or `kind/bug` item has shipped this session — proactive queue enrichment as a reliability gate: `§1e-chore-gate` added to `coord.md`. When `MEANINGFUL_PRS_THIS_SESSION==0` and the selected item is `kind/chore`, COORD scans `docs/design/*.md` for an unissued `🔲 Future` item, creates a GitHub issue for it, and claims that instead. Fallback: writes `meaningful_pr_guarantee_failed: true` to `state.json` and claims the chore normally. Fail-open on any error. `MEANINGFUL_PRS_THIS_SESSION` shell variable initialized to 0 at `§1a` startup. (PR #TBD, 2026-04-22)
