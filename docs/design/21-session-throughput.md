...
- 🔲 Housekeeping-streak auto-escalation: SM §4b must count consecutive sessions where `session_outcome == chore-only` in `metrics.md`. When the streak reaches 3 consecutive chore-only sessions, SM §4b must: (1) open a `kind/bug priority/high` issue titled "Housekeeping streak: 3 consecutive sessions with no meaningful work — pipeline broken"; (2) write `housekeeping_streak: 3` to `state.json` so COORD §1b reads it at next startup and immediately triggers autonomous vision synthesis before attempting any claim; (3) post to REPORT_ISSUE. A system that ships three sessions of chores in a row without self-detecting it is not a reliable autonomous system — it is a chore machine. The escalation must be automatic. ⚠️ Inferred from reliability lens: 3 consecutive housekeeping-only sessions are not detectable by the current loop; no mechanism converts the streak into an automatic recovery action.
- 🔲 COORD immediate queue refill when queue empties mid-session: when `coord.md §1e` finds 0 unclaimed `todo` items after completing an item (mid-session, before `session_item_limit` is reached), it currently exits to SM/PM. It should instead immediately attempt: (1) re-scan `docs/design/*.md` for unclaimed `🔲 Future` items not yet in the GitHub issue queue, (2) if found: create up to 5 issues and claim the first without waiting for SM to detect the empty queue. SM triggers vision synthesis only at session start or end — not inline. This means a queue that empties mid-session causes the rest of the `session_item_limit` budget to be wasted. The fix: COORD is the executor for inline refill; SM is the trigger for end-of-session synthesis. Mid-session refill requires no SM involvement. ⚠️ Inferred from reliability lens: a reliable system ships at least one meaningful PR every single run without exception; queue-empty mid-session currently wastes remaining budget.
- 🔲 Stuck-item detection and abandonment: an item that has been QA-rejected or CI-failed in 3 or more distinct sessions without ever merging is a "stuck item" — it consumes session budget on every retry without ever shipping. There is no mechanism today to detect or break this loop. SM §4b must: (1) track a `failed_attempts` counter per issue number in `state.json`; (2) increment it each time a session ends with a QA-rejected or CI-failed PR for that issue; (3) when `failed_attempts == 3`: re-label the issue `blocked`, downgrade to `priority/low`, and post a comment "⚠️ Item has failed QA or CI in 3 sessions — deprioritised. Root cause diagnosis needed before re-attempt."; (4) COORD §1e must skip `blocked`-labelled items during claim. Without this, a single difficult item can drain session budget indefinitely — the system cannot tell the difference between "hard but making progress" and "structurally impossible given the current agent capabilities." ⚠️ Inferred from reliability lens: a reliable system ships at least one meaningful PR every single run without exception; a stuck item that recycles through every session prevents the budget from reaching new work.
- 🔲 ENG zero-diff detection: ENG can complete an implementation cycle without modifying any file outside `docs/aide/` — for example, when the item description is ambiguous, the agent concludes "nothing to change" and exits with only a metrics-style commit (or no commit at all). This produces a session that exits cleanly with no useful work. SM §4b has no mechanism to detect this pattern specifically. COORD §1e must: after ENG exits, verify the working branch has ≥1 changed file outside `{docs/aide/, .otherness/, _state-equivalent}` before creating a PR. If zero meaningful files changed: log "ENG produced zero-diff for item #N — aborting PR creation, re-queuing item with `blocked` label and comment 'ENG zero-diff: item may be ambiguous or already implemented — human review needed before re-attempt.'"; increment `failed_attempts` counter. A PR that only touches metrics or aide files is treated identically to a zero-diff — both are non-deliverables. Without this gate, the PR queue fills with empty or housekeeping-only PRs that QA must reject, wasting QA's session time on unproductive reviews. ⚠️ Inferred from reliability lens: sessions produce housekeeping PRs with no real feature content; the current loop has no mechanism to intercept a zero-diff result before it becomes a PR.
